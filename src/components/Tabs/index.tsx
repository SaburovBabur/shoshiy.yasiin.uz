...
function Tabs<T extends string>({ value, onChange, tabs }: TabsProps<T>) {
  ...
  const tabsRef = useRef<{ [key in T]: HTMLButtonElement }>(
    {} as { [key in T]: HTMLButtonElement }
  );
  ...
  return (
    <nav className="py-6 flex items-center select-none">
      <div
        style={{
        ...
        className={clsx(
          "boundary bg-white/10 rounded-md absolute top-0 left-0"
        )}
      />
      ...
      {tabs.map((tab, idx) => (
        <button
          ...
          onClick={handleClick}
          className={clsx(
            "z-10 font-medium duration-def hover:font-bold cursor-pointer text-[#828387] hover:text-white",
            {
              "!text-white": activeTabValue === tab.value,
